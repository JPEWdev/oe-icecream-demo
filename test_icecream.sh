...
do_build() {
    ...
    bitbake $IMAGE
    cp -r tmp/buildstats/* $THIS_DIR/results/$NAME
    )
}

rm -rf $THIS_DIR/results
mkdir -p $THIS_DIR/results/
do_build without-icecream
do_build with-icecream

$THIS_DIR/poky/scripts/buildstats-diff --diff-attr cputime $THIS_DIR/results/without-icecream $THIS_DIR/results/with-icecream > $THIS_DIR/results/cputime.txt
$THIS_DIR/poky/scripts/buildstats-diff --diff-attr walltime $THIS_DIR/results/without-icecream $THIS_DIR/results/with-icecream > $THIS_DIR/results/walltime.txt

TOTALS_FILE=$THIS_DIR/results/task_totals.txt
rm -f $TOTALS_FILE
for task in do_configure do_compile do_install do_package_write_rpm; do
    echo "$task:" >> $TOTALS_FILE
    $THIS_DIR/poky/scripts/buildstats-diff --only-task $task --diff-attr cputime $THIS_DIR/results/without-icecream $THIS_DIR/results/with-icecream | tail -2 >> $TOTALS_FILE
    $THIS_DIR/poky/scripts/buildstats-diff --only-task $task --diff-attr walltime $THIS_DIR/results/without-icecream $THIS_DIR/results/with-icecream | tail -2 >> $TOTALS_FILE
    echo "" >> $TOTALS_FILE
done

ELAPSED_WITHOUT_ICECREAM=$(grep '^Elapsed time:' $THIS_DIR/results/without-icecream/build_stats | grep -o '[0-9\.]\+')
ELAPSED_WITH_ICECREAM=$(grep '^Elapsed time:' $THIS_DIR/results/with-icecream/build_stats | grep -o '[0-9\.]\+')
CPU_WITHOUT_ICECREAM=$(grep '^CPU usage:' $THIS_DIR/results/without-icecream/build_stats | grep -o '[0-9\.]\+')
CPU_WITH_ICECREAM=$(grep '^CPU usage:' $THIS_DIR/results/with-icecream/build_stats | grep -o '[0-9\.]\+')
...
cat <<HEREDOC > $THIS_DIR/results/elapsed.txt
